...
import cameraImage from '@/assets/webp/IMAGE 2025-08-29 21:02:58_1756494183898.webp';
...
export function FloatingServiceForm({ className = '', specialist }: FloatingServiceFormProps) {
  ...
  return (
    <div className={`fixed bottom-3 right-3 ${isOpen ? 'z-[100]' : 'z-40'} ${className}`} ref={formRef}>
      ...
      {isOpen && (
        <div className="floating-service-form absolute bottom-0 right-0 w-72 sm:w-80 md:w-96 lg:w-[420px] max-w-[85vw] h-[75vh] max-h-[580px] md:max-h-[550px] bg-white rounded-2xl shadow-2xl border border-slate-200 overflow-hidden animate-in fade-in slide-in-from-bottom-4 duration-300 flex flex-col relative z-[110]">
      ...
      {!isOpen && (
        <button
          onClick={toggleForm}
          type="button"
          className={`${colors.bg} ${colors.hoverBg} ${colors.activeBg} text-white rounded-xl shadow-2xl hover:shadow-3xl transition-shadow duration-300 flex items-center justify-center space-x-2 md:space-x-3 px-3 md:px-4 py-2.5 md:py-3 group cursor-pointer select-none border border-white/20`}
          style={{ zIndex: 9999 }}
        >
          {/* Avatar Icon */}
          <div className="w-6 h-6 md:w-7 md:h-7 bg-white/20 rounded-full flex items-center justify-center flex-shrink-0">
            <MessageCircle className="w-3.5 h-3.5 md:w-4 md:h-4" />
          </div>
          {/* Text */}
          <span className="text-sm md:text-sm font-medium whitespace-nowrap">Offerte aanvragen</span>
        </button>
      )}
    </div>
  );
