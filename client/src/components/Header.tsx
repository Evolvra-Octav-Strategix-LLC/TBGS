...
export default function Header({ onOpenContactModal, specialist, useH2 = false }: HeaderProps) {
  ...
  return (
    <header className="bg-white shadow-sm sticky top-0 z-50 rounded-b-3xl">
      <div className="w-full max-w-screen-2xl mx-auto px-2 sm:px-4 md:px-6 lg:px-8 py-4 overflow-x-auto">
        <div className="flex items-center justify-between gap-2 sm:gap-4 md:gap-6 lg:gap-8 min-w-max">
          {/* Logo */}
          <Link href="/" className="flex-shrink-0" aria-label="TBGS BV Homepage">
            <div className="flex items-center space-x-3 cursor-pointer">
              <img src="/img/TBGS%20545x642_1754935848756.webp" alt="TBGS Logo" className="w-12 h-12 object-contain" />
              <div>
                {useH2 ? (
          ...
          {/* Desktop Navigation */}
          <nav className="hidden lg:flex items-center space-x-2 lg:space-x-4 xl:space-x-6 flex-shrink-0">
          ...
          <div className="lg:hidden flex items-center space-x-1 sm:space-x-2">
            <Link href="/offerte" className={`${colors.bg} text-white px-2 sm:px-3 py-2 rounded-lg font-semibold ${colors.hoverBg} transition-colors text-xs sm:text-sm [@media(max-width:380px)]:hidden`}>
              Offerte
            </Link>
            <button 
