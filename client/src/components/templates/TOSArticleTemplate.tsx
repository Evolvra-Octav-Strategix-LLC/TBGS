...
function getCityUrl(cityText: string, country: 'nl' | 'be'): { cityName: string; url: string } {
  // Extract city name (remove service descriptions like "onderhoud", "schilderwerk", etc.)
  const cityName = cityText
    .replace(/\s+(onderhoud|schilderwerk|gevelreiniging|renovatie|reparatie).*$/i, '')
...
interface TOSArticleTemplateProps {
  // SEO Props
  title: string;
  description: string;
  url: string;
...
export default function TOSArticleTemplate({
  title,
  description,
  url,
  heroTitle,
            ...
            {serviceAreas && (
              <div className="bg-gray-50 p-8 rounded-2xl mb-16">
                ...
                <p className="text-lg text-gray-700 mb-6">
                  Onze <strong>onderhoud specialisten</strong> zijn actief in heel Nederland en België:
                </p>
                      ...
                      {serviceAreas.netherlands.map((city, index) => {
                        ...
                        return (
                          <div key={index} className="flex items-center">
                            ...
                            <Link 
                              href={url} 
                              className="text-tbgs-green hover:text-green-700 font-medium transition-colors"
